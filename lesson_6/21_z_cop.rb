...
# review stay, dealer_move, and describe_hand for necessity,
# complexity, and appropriateness of side effects
def stay(hand)
  prompt "dealer stays"
  hand
end
...
def higher_score(player_hand_to_test, player_hand_to_compare)
  if !bust?(player_hand_to_test)
    sum_hand(player_hand_to_test) > sum_hand(player_hand_to_compare)
  end
end

def player_won?(hand_to_test, hand_to_compare)
  higher_score(hand_to_test, hand_to_compare) || bust?(hand_to_compare)
end
...
loop do
  ...
  sleep(2)
  system 'clear'
  if winning_reason(dealer_hand, player_hand)
    puts "BLACKJACK!!!!"
